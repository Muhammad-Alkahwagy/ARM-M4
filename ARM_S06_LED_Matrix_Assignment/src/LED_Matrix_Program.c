...
{
    //u8 ArrayLength = strlen(Copy_u8Array);
    u8 i,j,k;
    for(i = 0; i<=Copy_u8ArrayLength-8; i++)
    {
        for(k=i, j=0; k<i+8, j<8; k++, j++)
        {
            /*Disable all Columns*/
            LEDMatrix_voidDisableCols();
            /*Set Row value*/
            LEDMatrix_voidSetRowVlaues(Copy_u8Array[k]);
            /*Enable column*/
            GPIO_voidSetPinValue(LED_MATRIX_COL_PORT, Columns[j], LOW);
            /*Delay for 2.5 ms*/
            SYSTICK_voidSetBusyWaitus(2500);
        }
        SYSTICK_voidSetBusyWaitms(125);
    }
}
